...
//change-1
import DIRTY_FIELD from '@salesforce/schema/epms__Unit__c.epms__Dirty__c';
...
export default class BookingGrid extends LightningElement {
    ...
    renderBookingData() {
        if (this._mapBookingEntityIdRecord) {
            ...
            let a =this.template.querySelector('.slds-grid.slds-wrap.slds-p-around_x-small.grid')
             if(a){
                    a.addEventListener('contextmenu', function (e) {  
                        e.preventDefault(); 
                    }, false);
            }
            this._templateWidth = this.template.querySelectorAll("[class='slds-grid slds-wrap slds-p-around_x-small grid']")[0].offsetWidth;
    ...
    isValidBooking(selectedUnitElements) {
        if (this.isSelectedUnitBooked) {
            this.showNotification('', UnitsNotAvailable, 'error');
            return false;
        }

        if (this.selectedCellUniySubType.length > 0) {
            for (let index = 0; this.selectedCellUniySubType.length > index; index++) {
                let selectedCell = this.selectedCellUniySubType[index];
                let cellarr = selectedCell.getAttribute('data-id').split('_');
                let newBookinInfo = this._mapBookingEntityIdRecord[cellarr[1]];
                let data = {};
                data = this.template.querySelectorAll(`[data-id=cell_${newBookinInfo.entitytype_id}_${cellarr[2]}]`);
                if (data[0].innerText.trim() === "0") {
                    this.isSelectedUnitBooked = true;
                    this.showNotification('', UnitsUnavailability, 'error');
                    return false;
                }
            }
        }

        if (selectedUnitElements.includes(0) && this.selectedCellUnitType.length === 0) {
            this.showNotification('', UnitsUnavailability, 'error');
            return false;
        } else if (selectedUnitElements.includes(0) && this.selectedCellUnitType.length > 0) {
            this.showNotification('', UnitTypesNotAvailable, 'error');
            return false;
        } else if (this.selectedCellUnitType.length > 0 && this.selectedCellUniySubType.length > 0) {
            this.showNotification('', UnitsAndUnitTypeSelected, 'error');
            return false;
        }
        else if (this.selectedCellUnitType.length > 1) {
            let setUnitTypeId = new Set();
            for (let index = 0; this.selectedCellUnitType.length > index; ++index) {
                setUnitTypeId.add(this.selectedCellUnitType[index].parentElement.getAttribute('data-id'));
            }
            if (setUnitTypeId.size > 1) {
                this.showNotification('', MultipleUnitTypesValidation, 'error');
                return false;
            }
            return true;
        }
    ...
    selectedItemForBooking(selectedCell) {
        this._newBooking = [];
        let currentSetected = [];
        for (let iIndex = 0; iIndex < selectedCell.length; iIndex++) {
            let cell = selectedCell[iIndex];
            let cellarr = cell.getAttribute('data-id').split('_');
            let itemNew = new Object();
            itemNew.id = cellarr[1];
            itemNew.selectDate = this._columnDates[cellarr[2]].date;
            if (cell.innerText !== "")
                itemNew.unitCount = parseInt(cell.innerText.trim());
            else
                itemNew.unitCount = 0;

            currentSetected[iIndex] = itemNew;
        }
        for (let index = 0; index < currentSetected.length; index++) {
            let bookingId = currentSetected[index].id;
            let newBookinInfo = this._mapBookingEntityIdRecord[bookingId];
            let startDate = currentSetected[index].selectDate;
            let unitCount = currentSetected[index].unitCount;
            while (currentSetected[index] != null && bookingId === currentSetected[index].id) {
                if (unitCount > currentSetected[index].unitCount) {
                    unitCount = currentSetected[index].unitCount;
                }
                index++;
            }
            index--;
            let endDate = currentSetected[index].selectDate;
            let booking = {
                id: '',
                name: '',
                entitytype_id: newBookinInfo.entitytype_id,
                entitychild1_id: newBookinInfo.entitychild1_id,
                entitychild2_id: newBookinInfo.entitychild2_id,
                propertyId: newBookinInfo.propertyId,
                startdate: startDate.toString(),
                enddate: endDate.toString(),
                unitCount: unitCount
            };
            this._newBooking.push(booking);
        }
        let details = {};
        details.newBooking = JSON.parse(JSON.stringify(this._newBooking));
        details.eventDetails = { 'clientX': this.x1, 'clientY': this.y1 - window.scrollY };
    ...
    existingBookingClicked(event) {
        let bookingdata = this._mapBookingDataIdRecord[event.target.getAttribute('data-id')];
        if (event.which === 1) {//mouse left click
            let url = window.location.origin + '/' + bookingdata.id;
            window.open(url, '_Blank');
        } else {//mouse right click
            this.isExistingBookingClicked = true;

            bookingdata = JSON.parse(JSON.stringify(bookingdata));
            bookingdata.eventDetails = event;
            const onexistingbookingclicked = new CustomEvent("existingbookingclicked", {
                detail: bookingdata
            });
            // Dispatches the event.
            this.dispatchEvent(onexistingbookingclicked);
            event.stopPropagation();
        }
    }

    existingInterruptionClicked(event) {
        let interruptionData = this._mapBookingDataIdRecord[event.target.getAttribute('data-id')];
        if (event.which === 1) {//mouse left click
            let url = window.location.origin + '/' + interruptionData.id;
            window.open(url, '_Blank');
        } else {//mouse right click
            this.isExistingBookingClicked = true;

            interruptionData = JSON.parse(JSON.stringify(interruptionData));
            interruptionData.eventDetails = event;
            const onexistinginterruptionclicked = new CustomEvent("existinginterruptionclicked", {
                detail: interruptionData
            });
            // Dispatches the event.
            this.dispatchEvent(onexistinginterruptionclicked);
            event.stopPropagation();
        }
    }
